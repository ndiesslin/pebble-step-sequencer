...
The app renders all four drum voices and both synth voices into one 16-step, 8 kHz PCM cache before playback begins. The small speaker pump then only copies cached audio, while the beat-rate playback indicator avoids competing with it. At tempos where Pebble has room for a second bar cache (normally 100 BPM and above), sound-affecting edits are prepared in the background and switch at the next bar boundary without stopping playback; the app shows `PREPARING NEXT BAR`, then `QUEUED NEXT BAR`. At slower tempos, it safely falls back to `CHANGES NEXT START` rather than risk speaker underruns. A failed or preempted stream changes the header to `AUDIO ERROR` and stops transport. System speaker mute / Quiet Time applies normally; firmware controls it and this SDK revision does not expose its mute state to the app.
